// <copyright file="UserTeamMappingsHelper.cs" company="Microsoft">
...
namespace Microsoft.Teams.Apps.DIConnect.Helpers
{
    ...
    public class UserTeamMappingsHelper
    {
        ...
        public async Task SendUserTeamMappingsCardAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken)
        {
            ...
            var userTeamMappings = await this.teamUserPairupMappingRepository.GetAllAsync(userId);
            var userMappingDetails = new List<TeamPairUpData>();

            if (resourceGroupDetails != null && resourceGroupDetails.Any() && userTeamMappings != null && userTeamMappings.Any())
            {
                foreach (var userTeamEntity in userTeamMappings)
                {
                    ...
                    {
                        userMappingDetails.Add(new TeamPairUpData
                        {
                ...
                var configureUserMatchesCard = MessageFactory.Attachment(this.cardHelper.GetUserPairUpMatchesCard(userMappingDetails, userTeamMappings));
