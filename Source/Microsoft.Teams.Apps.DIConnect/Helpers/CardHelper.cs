...
namespace Microsoft.Teams.Apps.DIConnect.Helpers
{
    ...
    public class CardHelper
    {
        ...
        /// </summary>
        /// <param name="teamUserPairUpMappingEntities">User and Teams pair-up matches entity.</param>
        /// <param name="teamUserPairUpMappings">Teams pair up mapping entities.</param>
        /// <returns>User pair-up card attachment.</returns>
        public Attachment GetUserPairUpMatchesCard(IEnumerable<TeamPairUpData> teamUserPairUpMappingEntities, IEnumerable<TeamUserPairUpMappingEntity> teamUserPairUpMappings)
        {
            teamUserPairUpMappingEntities = teamUserPairUpMappingEntities ?? throw new ArgumentNullException(nameof(teamUserPairUpMappingEntities));
            teamUserPairUpMappings = teamUserPairUpMappings ?? throw new ArgumentNullException(nameof(teamUserPairUpMappings));

            if (!teamUserPairUpMappingEntities.Any() || !teamUserPairUpMappings.Any())
            {
                this.logger.LogInformation("User pair-up matches is empty or null.");
            ...
            var optedOutTeamsPairUpMatches = string.Join(",", teamUserPairUpMappings.Where(row => row.IsPaused).Select(id => id.TeamId));
            var userMatchesCardContents = new UserPairUpMatchesNotificationCardData()
            {
                ...
                TeamPairUpEntities = teamUserPairUpMappingEntities,
                CommaSeparatedTeamIds = optedOutTeamsPairUpMatches,
